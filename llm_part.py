...
def get_agent_action_simple(session, model_name="gpt-3.5",temperature=1.0, max_tokens=200, HAHAHA=""):
    # 模型API设置
...
def get_prompt_area(area_data, llm_prompt_weight, gnn_pred, index, city, year, city_dict):
    ...
    community_name = pd.read_csv("./data/CHI/index2name.csv")
    community_dict = {}
    ...
    prompt_tail = f"""I will give you the TGNN prediction of crime count of each kind of crime in next month of {community}.
...
def llm_area_pred_func(area_data: np.array, llm_prompt_weight, gnn_pred, index, city, year, city_dict, openai_key):  #model为gpt-3.5-turbo, temperature=0, max_tokens=200
    ...
    model_name="gpt-3.5", temperature=0, max_tokens=200, OPENAI_KEY=openai_key
    )
