...
gdf = gpd.read_file("/home/tangyinzhou/inter_crime_pred/data/CHI/community.geojson")
...
for i, row1 in gdf.iterrows():
    ...
    for j, row2 in gdf.loc[gdf.index != i].iterrows():
        ...
        if row1["geometry"].intersects(row2["geometry"]):
            # 输出或存储相邻对
            community1.append(row1["community"])
            community2.append(row2["community"])

data = pd.DataFrame({"Community1": community1, "Community2": community2})

data.to_csv(
    "/home/tangyinzhou/inter_crime_pred/data/CHI/CHI_community_neighbor.csv",
    index=False,
)
