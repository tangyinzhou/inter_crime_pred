...
for year in trange(2006, 2021):
    ...
    # 读取DataFrame
    df = pd.read_csv(
        "/home/tangyinzhou/inter_crime_pred/data/NYC/crime_{0}.csv".format(year)
    )  # 替换为你的数据文件路径
    ...
    # 打印结果
    df = df.dropna()
    df.to_csv("/home/tangyinzhou/inter_crime_pred/data/NYC/crime_{0}.csv".format(year),index = False)
